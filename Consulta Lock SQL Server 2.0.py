...
class LockQueryApp:
    def __init__(self, root):
        ...
        self.connection_string = "DRIVER={SQL Server};SERVER=192.168.0.1;UID=sbosuporte;PWD=Pr0v1nc14"
    ...
    def create_widgets(self):
        ...
        self.kill_button = tk.Button(button_frame, text="Matar Processo", command=self.kill_process, state=tk.DISABLED)
    ...
    def kill_process(self):
        ...
        if selected_item:
            session_id = self.tree.item(selected_item, 'values')[1]
            result = self.kill_process_by_session_id(session_id)

            if result:
                messagebox.showinfo("Processo Matado", "O processo foi morto com sucesso.")
            else:
                messagebox.showerror("Erro", "Não foi possível matar o processo.")

    def kill_process_by_session_id(self, session_id):
        try:
            self.connection = pyodbc.connect(self.connection_string)
            self.cursor = self.connection.cursor()

            kill_query = f"KILL {session_id}"
            self.cursor.execute(kill_query)

            self.display_results([("Processo morto com sucesso",)], clear=True, color="green")
            return True

        except pyodbc.Error as e:
            self.display_results([("Erro ao matar processo:", str(e))], clear=True, color="red")
            return False
